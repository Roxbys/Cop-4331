...
class Robot:
  ...
  def getValidPaths(self):
    ...
    formatPaths = list()
    lastGreen = 0
    while(self.readEncoders()[1] < initEncoders[1] + 3100):
      self.motors(-50, 50)
    ...
    for record in recordPaths:
      if(record < 150 or record > 2700):
        ...
        formatPaths.append(("backward", record))
      elif(record > 1900 and record < 2700):
        formatPaths.append(("right",record))
    return formatPaths
         
  def getbaseline(self,baseline):
    grabEncoders = self.readEncoders()
    baseline = math.fabs(baseline + grabEncoders[0] - grabEncoders[1])
    print(baseline)
    return baseline
  ...
  def goForwardtwo(self):
    initEncoders = self.readEncoders()
    diff = 0
    while(self.checkGreen()):
      x = self.maxSpeed
      self.motors(x, x)
      newEncoders = self.readEncoders()
      leftEncoder = newEncoders[0] - initEncoders[0]
      rightEncoder = newEncoders[1] - initEncoders[1]
      diff = math.fabs(leftEncoder - rightEncoder)
      while(diff > 2 and self.checkGreen()):
        x -= 1
        if(rightEncoder > leftEncoder):
          self.motors(self.maxSpeed, x)
          time.sleep(1.0/1000.0)
        elif(leftEncoder > rightEncoder):
          self.motors(x, self.maxSpeed)
          time.sleep(1.0/1000.0)
  '''
  '''   
